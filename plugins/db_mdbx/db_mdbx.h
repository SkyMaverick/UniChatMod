...
#define trace_dbg(fmt, ...)                                                         \
    {                                                                               \
        app->app.log((ucm_plugin_t*)(UniDB), UCM_TYPE_LOG_DEBUG, fmt, __VA_ARGS__); \
    }
#define trace_inf(fmt, ...)                                                        \
    {                                                                              \
        app->app.log((ucm_plugin_t*)(UniDB), UCM_TYPE_LOG_INFO, fmt, __VA_ARGS__); \
    }
#define trace_err(fmt, ...)                                                         \
    {                                                                               \
        app->app.log((ucm_plugin_t*)(UniDB), UCM_TYPE_LOG_ERROR, fmt, __VA_ARGS__); \
    }
...
enum { DB_VALUE_BYTE, DB_VALUE_WORD, DB_VALUE_DWORD, DB_VALUE_QWORD, DB_VALUE_STRING, DB_VALUE_BLOB };

enum { DB_FLAG_DONTCLOSE = 1 };
...
{
    MDBX_txn* res = 0;
    int rc        = mdbx_txn_begin(db->mdbx.env, NULL, (db->flags & UCM_FLAG_ROPROF) ? MDBX_RDONLY : 0, &res);
    // TODO exception
    UNUSED(rc);
    return res;
}

static inline uint64_t MakeLong(uint32_t x, uint32_t y) { return (((uint64_t)(x)) << 32) | (y); }
