...
static int __log_cmp_helper(const MDBX_val* a, const MDBX_val* b)
{
    // TODO make
    UNUSED(a);
    UNUSED(b);
...
static void cb_mdbx_timer(uv_timer_t* timer) { mdbx_db_flush(true); }
...
static void cb_create_backup(uv_fs_t* open_req)
{
    // TODO
...
static UCM_RET mdbx_core_map(void)
{
    ...
    int rc = mdbx_env_set_geometry(UniDB->mdbx.env, -1, 1ul << 20, 512ul << 20, 1ul << 20, 512ul << 10, -1);
    if (rc != MDBX_SUCCESS)
...
static inline UCM_RET mdbx_core_load(void)
{
    ...
    if (txn_tmp) {
        ...
        if ((mdbx_dbi_open(txn_tmp, DBTABLE_NAME_GLOBAL, flags | MDBX_INTEGERKEY, &(UniDB->mdbx.dbi_global)) ==
             MDBX_SUCCESS) &&
            (mdbx_dbi_open(txn_tmp, DBTABLE_NAME_SETTINGS, flags | MDBX_INTEGERKEY, &(UniDB->mdbx.dbi_settings)) ==
             MDBX_SUCCESS) &&
            (mdbx_dbi_open(txn_tmp, DBTABLE_NAME_EVENTS, flags | MDBX_INTEGERKEY, &(UniDB->mdbx.dbi_events)) ==
             MDBX_SUCCESS) &&
            (mdbx_dbi_open(txn_tmp, DBTABLE_NAME_CONTACTS, flags | MDBX_INTEGERKEY, &(UniDB->mdbx.dbi_contacts)) ==
             MDBX_SUCCESS) &&
            (mdbx_dbi_open_ex(txn_tmp, DBTABLE_NAME_LOGS, flags | MDBX_INTEGERKEY, &(UniDB->mdbx.dbi_logs),
                              __log_cmp_helper, NULL) == MDBX_SUCCESS)) {
            uint32_t kVal = 1;
...
static void mdbx_core_unload(void)
{
    trace_dbg("%s\n", "Unload MDBX");
...
static inline void dbi_core_release(void)
{
