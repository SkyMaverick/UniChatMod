                ...
                u32char_t**     ret)
{
    ...
    if ( str ) {
        if ( str_len <= 0 ) {
            ...
            buf_len = str_len;
        *ret = ucm_kzmalloc ((buf_len + 1) * U32CHAR_SIZE);
        if (*ret) {
            for (;;){
                int64_t dec_len = 0;
                ...
                if (dec_len < buf_len) {
                    if ( ucm_krealloc ( (void**)ret, (dec_len + 1) * U32CHAR_SIZE) ) {
                       ucm_kfree_null(*ret);
                       return 0;
                    } else
                        return dec_len;
                ...
                u8char_t**       ret)
{
    ...
    if ( str && *ret ) {
        ...
        *ret = (u8char_t*)ucm_strdup (str);
        ...
        if ( ucm_krealloc ( (void**) ret, (enc_len + 1) * U8CHAR_SIZE) ) {
            ucm_kfree_null (*ret);
            return 0;
        }
    }
             ...
             u32char_t* str2)
{
    ...
    if (str1 && str2) {
        ...
        result = ucm_kzmalloc ((str1_len + str2_len + 1) * U32CHAR_SIZE);
              ...
              va_list  va)
{
    ...
    // copy to buffer
    buf = ucm_kzmalloc ((buf_len + 1) * U32CHAR_SIZE);
    if ( buf ) {
        u32char_t* p = buf;
        for (size_t i = 0; i < num; i++) {
                 ...
                 va_list  va)
{
    ...
    // copy to buffer
    buf = ucm_kzmalloc (buf_len * U32CHAR_SIZE);
    if ( buf ) {
        u32char_t* p = buf;
        for (size_t i = 0; i < num; i++) {
            u32char_t* tmp_buf = va_arg (va, u32char_t*);
            ...
            u32char_t* sstr)
{
    if (str && sstr) {
        ...
        int64_t* d = ucm_kzmalloc (M * sizeof(int64_t));
