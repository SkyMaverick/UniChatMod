...
static void _mq_flush(mq_block_t* h)
{
    h->head  = 0;
    h->tail  = 0;
    h->count = 0;
...
int mq_push(mq_block_t* h, uint32_t id, uintptr_t ctx, uint32_t x1, uint32_t x2)
{
    if (h) {
...
int mq_pop(mq_block_t* h, uint32_t* id, uintptr_t* ctx, uint32_t* x1, uint32_t* x2)
{
    if (h) {
        UniAPI->sys.mutex_lock(h->mtx);
        if (h->count > 0) {
...
int mq_noempty(mq_block_t* h) { return h->count > 0 ? 1 : 0; };

void mq_free(mq_block_t* h)
{
    UniAPI->sys.mutex_lock(h->mtx);
    _mq_flush(h);
    UniAPI->sys.mutex_unlock(h->mtx);
