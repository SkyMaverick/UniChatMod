...
#if defined(UCM_OS_BSD4) || defined(UCM_OS_DARWIN) || defined(UCM_OS_LINUX) || defined(UCM_OS_ANDROID) || \
    defined(UCM_OS_CYGWIN) || defined(UCM_OS_MSYS)
...
enum { UCM_LOOP_SYSTEM = 0, UCM_LOOP_NETWORK = 1 };
// *********************************************************
//      BASE STRUCTURES
// *********************************************************

// Global flags ---------------
enum { UCM_FLAG_ROPROF = 1 << 0, UCM_FLAG_NEWPROF = 1 << 1, UCM_FLAG_CHECKPROF = 1 << 2 };
...
// Messages -------------------
enum { UCM_FLAG_MSG_MULTYCAST = 1 << 0, UCM_FLAG_MSG_CRYPTO = 1 << 1, UCM_FLAG_MSG_ALERT = 1 << 2 };
...
typedef struct {
    ...
    ucm_dbval_t* (*get_setting)(HCONTACT contact, wchar_t* pid, const char* pName, ucm_dbval_t defVal);
...
typedef struct _ucm_functions_s {
    ...
    struct {
        ...
        void (*mainloop_hook_attach)(
            void (*callback)(uint32_t id, uintptr_t ev_ctx, uint32_t x1, uint32_t x2, void* ctx),
            void* ctx,
            uint32_t mask);
        void (*mainloop_hook_detach)(
            void (*callback)(uint32_t id, uintptr_t ev_ctx, uint32_t x1, uint32_t x2, void* ctx));
...
LIBUCM_API const size_t ucm_core_info(uint8_t mode, void* mem, size_t mem_size, ucm_cargs_t* args);
...
typedef const size_t (*ucm_cinfo_func)(uint8_t mode, void* mem, size_t mem_size, ucm_cargs_t* args);
