...
uintptr_t osal_thread_create(void* (*func)(void* ctx), void* ctx)
{
...
int osal_thread_join(uintptr_t tid) { return uv_thread_join((uv_thread_t*)tid); }
...
void osal_mutex_lock(uintptr_t _mtx) { uv_mutex_lock((uv_mutex_t*)_mtx); }
int osal_mutex_trylock(uintptr_t _mtx) { return uv_mutex_trylock((uv_mutex_t*)_mtx); }
void osal_mutex_unlock(uintptr_t _mtx) { uv_mutex_unlock((uv_mutex_t*)_mtx); }
...
void osal_cond_wait(uintptr_t _cond, uintptr_t _mtx) { uv_cond_wait((uv_cond_t*)_cond, (uv_mutex_t*)_mtx); }
void osal_cond_signal(uintptr_t _cond) { uv_cond_signal((uv_cond_t*)_cond); }
void osal_cond_broadcast(uintptr_t _cond) { uv_cond_broadcast((uv_cond_t*)_cond); }
...
int osal_rwlock_trywlock(uintptr_t _rwl)
{
...
void osal_rwlock_unlock(uintptr_t _rwl)
{
